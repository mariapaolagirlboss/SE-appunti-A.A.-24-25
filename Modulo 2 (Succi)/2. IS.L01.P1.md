...
2. **Relationship between classes**
	- Associazione: relazione tra istanze ci classi (cioè tra gli oggetti). Indica che gli oggetti di una classe sono collegati agli oggetti di un'altra classe e che questi oggetti devono "conoscersi" per interagire tra di loro. 
		...
		- **Ruoli**: un'associazione ha due ruoli. Il ruolo è una direzione dell'associazione. Il ruolo è: **esplicitamente etichettato** (si assegna un nome descrittivo) ed **implicitamente nominato come la classe target** (se non viene specificato alcun nome di ruolo, si utilizza il nome della classe che partecipa all'associazione).